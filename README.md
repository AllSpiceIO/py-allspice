...
Check the [examples directory](https://github.com/AllSpiceIO/py-allspice/tree/main/packages/allspice/examples)
...
Note that the fields of the User, Organization,... classes are dynamically created at runtime, and thus not visible during development. Refer to the AllSpice API documentation for the fields names.
...
`http://localhost:3000`, and an admin-user token at `.token` in the repo root. The admin user
...
```sh
uv run pytest -k <specifier for test> --snapshot-update
```
