...
from allspice.apiobject import CommitStatusState, PullReview, Util
...
def test_create_design_review_review(instance):
    ...
    review = dr.create_review(
        body="New Review",
        comments=[PullReview.ReviewComment("Comment within review", "new_file.txt")],
    )

    assert review.body == "New Review"
    assert review.comments_count == 1
    assert review.state == PullReview.ReviewEvent.PENDING
...
def test_submit_design_review_review(instance):
    ...
    assert review.state == PullReview.ReviewEvent.PENDING
    dr.submit_review(review.id, PullReview.ReviewEvent.COMMENT, body="New Body")
    del review
    review = dr.get_reviews()[0]
    assert review.state == PullReview.ReviewEvent.COMMENT
